...
def test_projected_bbox_is_derived_from_transform_and_shape():
    attrs = granule_attrs(
        epsg=32620, shape=SHAPE, transform=TRANSFORM, levels=5, array_name="B04"
    )
...
def test_layout_transform_is_the_step_from_the_level_it_derives_from():
    ...
    layout = multiscale_layout(levels=5, array_name="B04")
    assert len(layout) == 5
    assert layout[0] == {"asset": "multiscales/0/B04"}
    assert layout[1] == {
        "asset": "multiscales/1/B04",
        "derived_from": "multiscales/0/B04",
        "transform": {"scale": [2, 2]},
    }
    assert layout[4]["derived_from"] == "multiscales/3/B04"
...
def test_single_level_layout_has_no_derived_entries():
    assert multiscale_layout(levels=1, array_name="B04") == [
        {"asset": "multiscales/0/B04"}
    ]


def test_layout_paths_are_relative_to_the_group_carrying_the_attributes():
    """The attributes go on the band group; the levels live two below it --
    the ``multiscales`` group, then a group per level. A bare ``"0"`` would
    name a sibling that does not exist."""
    layout = multiscale_layout(levels=3, array_name="B04")
...
def test_an_asset_names_the_array_inside_the_level_group():
    """A level group is not itself the data. The convention resolves an
    ``asset`` path to an array, so a layout stopping at the group names
    something that holds the pyramid rather than something that is a level
    of it."""
    layout = multiscale_layout(levels=2, array_name="Fmask")
    assert [entry["asset"] for entry in layout] == [
        "multiscales/0/Fmask",
        "multiscales/1/Fmask",
    ]
